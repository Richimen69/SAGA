...
export default function TramiteRow({ tramite }) {
  ...
  const fechaColorClass = useMemo(() => {
    ...
    const hoy = new Date();
    const diferenciaDias = Math.floor((hoy - fechaTramite) / (1000 * 60 * 60 * 24));

    return diferenciaDias > 30 ? "text-red-600 bg-red-100 rounded-lg" : "text-gray-600";
  ...
  const handleRowClick = () => {
    navigate(`/vistaprevia`, { state: { id: tramite.id } });
  };
  ...
  return (
    ...
    >
      <td className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap">{tramite.folio}</td>
      <td className="px-4 py-3 font-medium text-gray-900">{tramite.beneficiario_nombre}</td>
      <td className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap">{tramite.numero_fianza}</td>
      <td className="px-4 py-3 text-gray-600">{tramite.cliente_nombre}</td>
      <td className="px-4 py-3">
        <p className={`p-1 ${fechaColorClass} text-left`}>{format(
                        parseISO(tramite.fecha),
                        "dd/MM/yyyy"
                      )}</p>
      </td>
      ...
      <td className="relative px-4 py-3 text-gray-600 whitespace-nowrap text-center group">
        <div className="flex flex-col items-center">
          <span className={statusStyles[tramite.estatus_nombre] || "bg-gray-200 text-gray-600 px-3 py-1 rounded-full text-base font-semibold tex"}>
      ...
      <td className="px-4 py-3 text-gray-600 text-left whitespace-nowrap">{tramite.tipo_proceso}</td>
